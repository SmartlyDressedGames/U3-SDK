...
namespace SDG.Unturned
{
	...
	public static class InteractableSafezone_NetMethods
	{
		...
		{
			InteractableSafezone netObj = voidNetObj as InteractableSafezone;
			if (netObj == null)
			{
				context.LogWarning($"expected target instance to be type InteractableSafezone, but was {voidNetObj.GetType().Name}");
				return;
			}
		...
		[NetInvokableGeneratedMethod(nameof(InteractableSafezone.ReceivePowered), ENetInvokableGeneratedMethodPurpose.Read)]
		public static void ReceivePowered_Read(in ClientInvocationContext context)
		{
			...
			if (netObj == null)
			{
				context.LogWarning($"expected target instance with net id {netId} to be type InteractableSafezone, but was {voidNetObj.GetType().Name}");
				return;
			}
			System.Boolean newPowered;
#if LOG_INVOKE_READ_ERRORS
		...
		[NetInvokableGeneratedMethod(nameof(InteractableSafezone.ReceivePowered), ENetInvokableGeneratedMethodPurpose.Write)]
		public static void ReceivePowered_Write(NetPakWriter writer, System.Boolean newPowered)
		{
			writer.WriteBit(newPowered);
		}
		[NetInvokableGeneratedMethod(nameof(InteractableSafezone.ReceiveToggleRequest), ENetInvokableGeneratedMethodPurpose.Read)]
		...
		[NetInvokableGeneratedMethod(nameof(InteractableSafezone.ReceiveToggleRequest), ENetInvokableGeneratedMethodPurpose.Write)]
